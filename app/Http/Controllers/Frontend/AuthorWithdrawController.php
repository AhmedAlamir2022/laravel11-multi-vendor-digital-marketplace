    ...
    function store(Request $request): RedirectResponse
{
    ...
    if (!$withdrawGateway) {
        throw ValidationException::withMessages([
            'amount' => __('No withdraw gateway configured. Please set it up first from payouts tap on from profile page.')
        ]);
    }

    if ($request->amount < $withdrawGateway->minimum_amount) {
        throw ValidationException::withMessages([
            'amount' => __('The minimum amount for withdraw is :amount', ['amount' => $withdrawGateway->minimum_amount])
        ]);
    }

    if ($request->amount > $withdrawGateway->maximum_amount) {
        throw ValidationException::withMessages([
            'amount' => __('The maximum amount for withdraw is :amount', ['amount' => $withdrawGateway->maximum_amount])
        ]);
    }

    if ($request->amount > user()->balance) {
        throw ValidationException::withMessages([
            'amount' => __('You don\'t have enough balance')
        ]);
    }

    if (user()->withdraws()->whereStatus('pending')->exists()) {
        throw ValidationException::withMessages([
            'amount' => __('You already have a pending withdraw request')
        ]);
    }
